...
workList가 빌 때까지 반복한다.
    ...
    현재 time보다 1을 더해서 shortestTime을 정의한다.
    ...
    걸리지 않으면(현재 실행가능한 job이 없으면)
        job을 탐색할 때, 만약 현재 shortestTime보다 작은 요청시간이 있으면 shortestTime을 갱신한다.
        job 탐색이 다 끝났는데도 걸린 게 없으면, time을 shortestTime-time만큼 늘린다.
...
function solution(jobs) {
  ...
  while (workList.length > 0) {
    let request = -1;
    let shortestTime = time + 1;
    for (let i = 0; i < workList.length; i++) {
      if (workList[i][0] <= time) {
        request = i;
        break;
      }
      if (workList[i][0] <= shortestTime) {
        shortestTime = workList[i][0];
      }
    }
    if (request === -1) {
